...
export class MenuService {
  private menusSubject = new BehaviorSubject<Array<Menu>>([
    ...
    new Menu({name: '首页', icon: 'fab fa-asymmetrik', url: 'dashboard', roles: [Menu.ROLE_ADMIN, Menu.ROLE_COMMON, Menu.ROLE_REPAIR, Menu.ROLE_MANAGER]}),
    new Menu({name: '类型管理', icon: 'fab fa-audible', url: 'type', roles: [Menu.ROLE_ADMIN]}),
    new Menu({name: '部门管理', icon: 'far fa-building', url: 'department', roles: [Menu.ROLE_ADMIN]}),
    // tslint:disable-next-line:max-line-length
    new Menu({name: '设备管理', icon: 'fas fa-clipboard-list', url: 'equipment', roles: [Menu.ROLE_ADMIN, Menu.ROLE_MANAGER, Menu.ROLE_COMMON, Menu.ROLE_REPAIR]}),
    new Menu({name: '人员管理', icon: 'far fa-address-card', url: 'user', roles: [Menu.ROLE_ADMIN]}),
